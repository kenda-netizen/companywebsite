      ...
      <div className="container-page grid gap-8 py-10 md:grid-cols-3">
        ...
        <section>
          ...
          <ul className="mt-3 space-y-2 text-sm">
            {quickLinks.map((link) => (
              <li key={link.href}>
                <Link href={link.href} className="text-text-secondary hover:text-brand-primary">
                  {link.label}
                </Link>
              </li>
            ))}
        ...
        <section>
          ...
          <p className="mt-3 text-sm text-text-secondary">
            טלפון:{' '}
            <a href={`tel:${business.phone}`} className="font-semibold text-brand-primary hover:text-brand-secondary">
              {business.phone}
            </a>
          </p>
          <p className="mt-2 text-sm text-text-secondary">
            וואטסאפ:{' '}
            <a href={business.whatsappLink} target="_blank" rel="noreferrer" className="font-semibold text-brand-primary hover:text-brand-secondary">
              מעבר לצ׳אט
            </a>
          </p>
          ...
          <Link
            ...
            target="_blank"
            rel="noreferrer"
            className="mt-4 inline-flex rounded-brand border border-border-strong px-4 py-2 text-sm font-semibold text-text-secondary transition hover:border-brand-primary hover:text-brand-primary"
          >
            נווט ב-Waze
          </Link>
        </section>
